...
class HeuristicaConstructivaAleatoria {
    ...
    cumpleRestriccionesDurasyPoliticas(solution, candidate, instance, occupantMap) {
    ...
    fijarAsignacion(solution, candidate, instance) {
        const { patientAssigns, roomOccupancy, OTUsage, surgeonUsage } = solution;
        const { patientId, day: d, roomId: r, OTid: t } = candidate;
        const p = instance.patients.find(pac => pac.id === patientId);
        const stay = p.length_of_stay;
    ...
    heuristicaConstructivaAleatoria(instance) {
        const D = instance.days;
        const rooms = instance.rooms;
        const OTs = instance.operating_theaters;
        ...
        const asignarConMRV = (pool, pendientesArray, esOpcional = false) => {
            ...
            while (pendientes.size > 0) {
                ...
                while (candidatos.length > 0) {
                    ...
                    if (this.cumpleRestriccionesDurasyPoliticas(solution, eleccion, instance, occupantMap)) {
                        // Encaja: asignar y actualizar
                        this.fijarAsignacion(solution, eleccion, instance);
                        ...
                        pool = pool.filter(c => this.cumpleRestriccionesDurasyPoliticas(solution, c, instance, occupantMap));
                        assigned = true;
                        break;
                    } else {
    ...
    construirRequerimientosEnfermeras(solution, instance) {
        const D = instance.days;
    ...
    asignarEnfermeras(solution, instance) {
        const D = instance.days;
        ...
        // 2) Requerimientos
        const RN = this.construirRequerimientosEnfermeras(solution, instance);
    ...
    ejecutarHeuristica(instance, maxRetries = 10) {
        ...
        for (let intento = 0; intento < maxRetries; intento++) {
            solPacientes = this.heuristicaConstructivaAleatoria(instance);
        ...
        const solEnfermeras = this.asignarEnfermeras(solPacientes, instance);
...
class HeuristicaDummy {
    ejecutarHeuristica(instance) {
        return {
            patients: instance.patients.map(p => ({ id: p.id, admission_day: "none" })),
            nurses: instance.nurses.map(n => ({ id: n.id, assignments: [] }))
        };
    }
}
...
class Heuristica {
    ...
    ejecutarHeuristica(instance, maxRetries) {
        return this.strategy.ejecutarHeuristica(instance, maxRetries);
    }
}
